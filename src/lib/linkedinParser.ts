 ...
 *
 * The parser classifies each PDF text item by its font size (linkedinLines.ts),
 * groups them into lines, segments by section header, then extracts structured
 * data: the header and sidebar (linkedinHeader.ts), the experiences
 * (linkedinExperience.ts) and the education (here).
 *
...
import { FontRole, joinAll, spaceSentences, splitColumns, tokensToLines, type Line, type Token } from './linkedinLines';
import { parseLanguages, parsePersonalInfo, parseSkills } from './linkedinHeader';
import { parseExperiences } from './linkedinExperience';

export type { Token } from './linkedinLines';
